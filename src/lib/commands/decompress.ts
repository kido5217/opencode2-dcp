...
import type { CompressionBlock, PruneMessagesState, SessionState } from "../types.ts";
...
/**
 * v2 port of v1 `lib/commands/decompress.ts` (restores user-decompressed
 * compressions: `/dcp decompress <n>`).
 *
 * Seam adaptations: the v1 `client` (fetch/persist/notify) is replaced by
 * `storage` (v2 persistence is injected) and a `sendIgnoredMessage` callback
 * (v1 sent a synthetic no-reply prompt through the client; the v2 channel is
 * wired in the notification ticket — handlers take the sender by DI so tests
...
export async function handleDecompressCommand(ctx: DecompressCommandContext): Promise<void> {
  ...
  if (args.length > 1) {
    await sendIgnoredMessage("Invalid arguments. Usage: /dcp decompress <n>", params);
    return;
  }
  ...
  if (activeBlocks.length === 0) {
    ...
    await sendIgnoredMessage(`Compression ${target.displayId} is not active.`, params);
    return;
  }
