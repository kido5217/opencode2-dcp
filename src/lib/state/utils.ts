...
import { isIgnoredUserMessage, messageHasCompress } from "../messages/query.ts";
import { countTokens } from "../token-utils.ts";

/**
 * v2 port of v1 `lib/state/utils.ts`.
 *
 * v2 deltas:
 * - `isMessageCompacted` no longer compares message timestamps (v2 request
 *   messages carry none); compaction is event-driven (`session.compaction.ended`
 *   resets the state), so only active-block coverage marks a message compacted.
 * - `findLastCompactionTimestamp` is dropped (no summary-flagged messages in
 *   the request shape).
 * - `countTurns` counts assistant messages (each assistant message is one
 *   model step in the v2 request shape; v1 counted `step-start` parts).
 * - `isSubAgentSession` takes a session-info getter (the v2
 *   `ctx.session.get` returns the info directly and throws on missing
 *   sessions) instead of the v1 client.
 */

export const isMessageCompacted = (state: SessionState, msg: DcpMessage): boolean => {
  const pruneEntry = msg.id ? state.prune.messages.byMessageId.get(msg.id) : undefined;
...
export function loadPruneMessagesState(
  ...
  if (persisted.byMessageId && typeof persisted.byMessageId === "object") {
    for (const [messageId, entry] of Object.entries(persisted.byMessageId)) {
      ...
      const allBlockIds = Array.isArray(entry.allBlockIds)
        ? [
            ...new Set(
              entry.allBlockIds.filter((id): id is number => Number.isInteger(id) && id > 0),
            ),
          ]
        : [];
      const activeBlockIds = Array.isArray(entry.activeBlockIds)
        ? [
            ...new Set(
              entry.activeBlockIds.filter((id): id is number => Number.isInteger(id) && id > 0),
            ),
          ]
        : [];
      state.byMessageId.set(messageId, { tokenCount, allBlockIds, activeBlockIds });
    }
  }
  ...
  if (persisted.blocksById && typeof persisted.blocksById === "object") {
    for (const [blockIdStr, block] of Object.entries(persisted.blocksById)) {
      ...
      state.blocksById.set(blockId, {
        blockId,
        runId:
          typeof block.runId === "number" && Number.isInteger(block.runId) && block.runId > 0
            ? block.runId
            : blockId,
        active: block.active === true,
        deactivatedByUser: block.deactivatedByUser === true,
        compressedTokens:
          typeof block.compressedTokens === "number" && Number.isFinite(block.compressedTokens)
  ...
  if (persisted.activeByAnchorMessageId && typeof persisted.activeByAnchorMessageId === "object") {
    for (const [anchorMessageId, blockId] of Object.entries(persisted.activeByAnchorMessageId)) {
      if (typeof blockId !== "number" || !Number.isInteger(blockId) || blockId < 1) continue;
...
export function collectTurnNudgeAnchors(messages: DcpMessage[]): Set<string> {
  const anchors = new Set<string>();
  let pendingUserMessageId: string | null = null;

  for (let i = messages.length - 1; i >= 0; i--) {
    const message = messages[i];
    if (messageHasCompress(message, messages)) {
      break;
    }
    if (message.role === "user") {
      if (message.id && !isIgnoredUserMessage(message)) {
        pendingUserMessageId = message.id;
      }
      continue;
    }
    if (message.role === "assistant" && message.id && pendingUserMessageId) {
