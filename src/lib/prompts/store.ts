...
import { findOpencodeDir } from "../../config.ts";
import { SYSTEM as SYSTEM_PROMPT } from "./system.ts";
import { COMPRESS_RANGE as COMPRESS_RANGE_PROMPT } from "./compress-range.ts";
import { COMPRESS_MESSAGE as COMPRESS_MESSAGE_PROMPT } from "./compress-message.ts";
import { CONTEXT_LIMIT_NUDGE } from "./context-limit-nudge.ts";
import { TURN_NUDGE } from "./turn-nudge.ts";
import { ITERATION_NUDGE } from "./iteration-nudge.ts";
import { MANUAL_MODE_SYSTEM_EXTENSION, SUBAGENT_SYSTEM_EXTENSION } from "./extensions/system.ts";

/**
 * v2 port of v1 `lib/prompts/store.ts` — prompt resolution with per-level
 * overrides (project → config dir → global). The override directory layout
 * and normalization rules are verbatim; `findOpencodeDir` is reused from
 * `src/config.ts` (same walk-up semantics).
 */
